...
테스트는 두 면으로 나눴다. 첫 번째는 단일 화자 3,922문장이다. 같은 `file_name`, 같은 `ground_truth`를 기준으로 AWS Transcribe와 Whisper base, medium, large transcript를 모았다. 중심 데이터인 3,922문장은 직접 읽고 녹음한 내 음성 데이터였다. 집 안에서 비교적 조용하고 반향이 적은 서재에서 하루 50문장을 목표로 읽었고, 멀티 패턴 USB 콘덴서 마이크의 단일 지향성(cardioid) 수음 모드로 문장을 하나씩 WAV 파일로 남겼다. 누적 리딩과 녹음 시간은 거의 8시간에 가까웠고, 실험 설계부터 녹음, STT 실행, 결과 수집, metric 고정까지 약 3개월 동안 직접 리딩했다. 이 데이터는 모델별 평균 CER뿐 아니라 완전 인식률, threshold pass rate, paired delta, worst/disagreement examples를 계산하는 중심 데이터가 됐다.

두 번째는 금융 콜센터 3개 persona였다. PB 채권 주문 상담, 신생기업 대출 안내 상담, 자동차 보험료 할증 문의 상담처럼 숫자 입력 정확도가 중요한 상황을 팀원 몇 명과 30분 정도 짧게 녹음했다. 미팅룸에서는 개인 비용으로 준비한 멀티 패턴 USB 콘덴서 마이크를 양방향 수음 모드로 두어 1:1 대화의 양쪽 음성을 균형 있게 담아보려 했다. 약 30분 규모의 보조 케이스였기 때문에 provider ranking을 만들기 위한 데이터가 아니다. 대신 숫자 표기, 도메인 용어, 상담 흐름이 CER를 어떻게 흔드는지 보기 위한 case analysis다.
...
두 번째는 금융 콜센터를 가정한 3개 시나리오다. PB 채권 주문 상담, 신생기업 대출 안내 상담, 자동차 보험료 할증 문의 상담을 AWS, Azure, Clova, GCP 결과와 비교했다. 팀원 몇 명과 30분 정도 미팅하고 녹음한 간이 테스트였고, 콜센터 특성상 전화번호, 계좌번호, 금액, 날짜처럼 숫자 입력 정확도가 중요한 표현을 확인하는 데 초점을 뒀다. 미팅룸에서는 개인 비용으로 준비한 멀티 패턴 USB 콘덴서 마이크를 양방향(bidirectional) 수음 모드로 두어 1:1 대화의 양쪽 음성을 균형 있게 담아보려 했다. 이 데이터는 표본 수가 작기 때문에 provider ranking으로 쓰지 않는다. 대신 도메인 용어, 숫자 표기, 상담형 문맥에서 오류 양상이 어떻게 달라지는지 보는 case analysis로 다룬다.
...
블로그의 인터랙티브 영역은 독자가 threshold를 직접 바꿔보는 방식으로 설계한다. 기본 뷰는 단일 화자 데이터의 pass rate chart다. dataset을 call center로 바꾸면 Numeric ground truth, Azure, 10% threshold가 기본으로 잡히고, provider 목록에서도 Azure를 먼저 보여준다.
